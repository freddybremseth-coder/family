...
import { ArrowRight, CalendarDays, CheckCircle2, CreditCard, Database, FileText, Heart, Home, Loader2, LockKeyhole, ScanLine, ShieldCheck, ShoppingCart, X } from 'lucide-react';
...
export const LandingPageClean: React.FC<Props> = ({ onLogin, lang, setLang }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authMode === 'signup') {
        const cleanedFamilyName = familyName.trim();
        const familyId = `${slugifyFamily(cleanedFamilyName)}-${Date.now().toString(36)}`;
        ...
        setMessage(error ? error.message : `Sjekk e-posten din for bekreftelseslenke. Familie-ID: ${familyId}. Kontoen starter med ${demoMode === 'empty' ? 'tomt dashboard' : 'ufarlig random demo-data'}, aldri data fra andre brukere.`);
    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <main>
        ...
        <section id="plans" className="mx-auto max-w-7xl px-5 py-20">
          <div className="mb-10 max-w-2xl"><h2 className="text-3xl font-black tracking-tight md:text-4xl">Basic, Avansert og moduler.</h2><p className="mt-3 text-slate-600">Start enkelt. Utvid senere med moduler som passer familien eller bedriften.</p></div>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            {[
              ['Basic', 'Kalender, oppgaver, handleliste, familiemedlemmer, dokumenter og enkel økonomi.'],
              ['Avansert', 'Alt i Basic pluss bank, eiendeler, likviditet, kvitteringer, kontoutskrift og regninger.'],
              ['Moduler', 'Business/RealtyFlow, AI-skanning, Kalender Pro, dokumentlager, bank/eiendeler og fremtidige add-ons.'],
            ].map(([title, text]) => <div key={title} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h3 className="text-xl font-black">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{text}</p></div>)}
          </div>
        </section>
