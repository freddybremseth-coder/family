  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Schema</p><p className="mt-1 text-sm font-medium text-slate-700">{schema || 'public'}</p></div>
        <div className={`rounded-2xl p-4 ${legacyDetected ? 'bg-red-50' : 'bg-slate-50'}`}><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Project ref</p><p className={`mt-1 text-sm font-medium ${legacyDetected ? 'text-red-700' : 'text-slate-700'}`}>{legacyDetected ? 'Gammel Supabase blokkert' : 'Ingen gammel ref funnet'}</p></div>
      </div>
    </div>
  );
}
...
export const IntegrationsSettings: React.FC = () => {
  const allOk = SUPABASE_STATUS.familyUrlConfigured && SUPABASE_STATUS.familyKeyConfigured && SUPABASE_STATUS.realtyflowUrlConfigured && SUPABASE_STATUS.realtyflowKeyConfigured && SUPABASE_STATUS.donaAnnaUrlConfigured && SUPABASE_STATUS.donaAnnaKeyConfigured && !SUPABASE_STATUS.legacyProjectDetected;
  return (
    <div className="space-y-6">
      ...
      <div className={`rounded-2xl border p-4 ${allOk ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}>
        <div className="flex items-start gap-3">
          ...
          <div><p className={`font-bold ${allOk ? 'text-emerald-900' : 'text-amber-900'}`}>{allOk ? 'Alle hovedintegrasjoner er konfigurert' : SUPABASE_STATUS.legacyProjectDetected ? 'Gammel gratis Supabase er blokkert' : 'Én eller flere integrasjoner mangler key i build'}</p><p className={`mt-1 text-sm ${allOk ? 'text-emerald-700' : 'text-amber-700'}`}>Vite-miljøvariabler må være satt før build/redeploy. Family bruker schema {SUPABASE_STATUS.familySchema}; Olivia bruker schema {SUPABASE_STATUS.donaAnnaSchema}.</p></div>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <IntegrationCard title="FamilyHub" text="Egne familie-data, innlogging og app-state." url={SUPABASE_REFS.family} urlOk={SUPABASE_STATUS.familyUrlConfigured} keyOk={SUPABASE_STATUS.familyKeyConfigured} keyName={SUPABASE_STATUS.familyResolvedKeyName} keyLength={SUPABASE_STATUS.familyKeyLength} schema={SUPABASE_STATUS.familySchema} legacyDetected={SUPABASE_STATUS.familyLegacyProjectDetected} />
        <IntegrationCard title="RealtyFlow" text="Soleada, ZenEcoHomes og eiendomsprovisjoner." url={SUPABASE_REFS.realtyflow} urlOk={SUPABASE_STATUS.realtyflowUrlConfigured} keyOk={SUPABASE_STATUS.realtyflowKeyConfigured} keyName={SUPABASE_STATUS.realtyflowResolvedKeyName} keyLength={SUPABASE_STATUS.realtyflowKeyLength} legacyDetected={SUPABASE_STATUS.realtyflowLegacyProjectDetected} />
        <IntegrationCard title="Dona Anna / Olivia" text="Gårdsdrift, olje, inntekter og kostnader." url={SUPABASE_REFS.donaAnna} urlOk={SUPABASE_STATUS.donaAnnaUrlConfigured} keyOk={SUPABASE_STATUS.donaAnnaKeyConfigured} keyName={SUPABASE_STATUS.donaAnnaResolvedKeyName} keyLength={SUPABASE_STATUS.donaAnnaKeyLength} schema={SUPABASE_STATUS.donaAnnaSchema} legacyDetected={SUPABASE_STATUS.donaAnnaLegacyProjectDetected} />
      </div>
