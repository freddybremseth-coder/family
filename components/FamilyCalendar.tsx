...
export const FamilyCalendar: React.FC<Props> = ({ familyMembers, calendarEvents, setCalendarEvents, tasks, setTasks, userConfig, localEvents, setLocalEvents }) => {
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6"><div className="xl:col-span-2 space-y-6">
      ...
      <section className="card p-4 md:p-6"><div className="mb-6 flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><h3 className="section-title"><CalendarDaysIcon /> {dateLabel(selectedDate, userConfig.language, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</h3><p className="section-subtitle">{selectedDate === todayStr() ? 'I dag' : 'Plan for valgt dag'} · {activeMemberName}</p></div><div className="grid grid-cols-2 gap-2 w-full md:w-auto"><button onClick={() => { setEditingEvent(emptyEvent(selectedDate, defaultMemberId)); setShowEventForm(true); setShowTaskForm(false); }} className="btn-gradient justify-center min-h-[44px]"><Plus className="w-4 h-4" /> Hendelse</button><button onClick={() => { setEditingTask(emptyTask(selectedDate, defaultMemberId)); setShowTaskForm(true); setShowEventForm(false); }} className="min-h-[44px] rounded-xl bg-amber-500 px-4 py-2 text-sm font-bold text-white flex items-center justify-center gap-2"><Plus className="w-4 h-4" /> Oppgave</button></div></div>
        ...
        {showTaskForm && editingTask && <div className="mb-5 rounded-2xl border border-amber-100 bg-amber-50 p-4"><div className="mb-4 flex items-center justify-between"><div><h4 className="font-bold text-slate-900">{editingTask.id ? 'Rediger oppgave' : 'Ny oppgave'}</h4><p className="text-sm text-slate-600">Oppgaver kan også gjelde flere familiemedlemmer.</p></div><button onClick={() => setShowTaskForm(false)} className="rounded-xl p-2 hover:bg-white"><X className="h-4 w-4" /></button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><label className="md:col-span-2"><span className="text-sm font-semibold text-slate-700">Hva må gjøres?</span><input value={editingTask.description || ''} onChange={e => setEditingTask({ ...editingTask, description: e.target.value })} className={`${inputClass} mt-1`} /></label><label><span className="text-sm font-semibold text-slate-700">Dato</span><input type="date" value={editingTask.date || selectedDate} onChange={e => setEditingTask({ ...editingTask, date: e.target.value })} className={`${inputClass} mt-1`} /></label><label><span className="text-sm font-semibold text-slate-700">Prioritet</span><select value={editingTask.priority || 'Medium'} onChange={e => setEditingTask({ ...editingTask, priority: e.target.value })} className={`${inputClass} mt-1`}>{PRIORITIES.map(priority => <option key={priority} value={priority}>{PRIORITY_CONFIG[priority].label}</option>)}</select></label><div className="md:col-span-2"><p className="text-sm font-semibold text-slate-700 mb-2">Ansvarlige</p><div className="flex flex-wrap gap-2">{familyMembers.map(member => { const active = (editingTask.assignedToIds || []).includes(member.id); return <button type="button" key={member.id} onClick={() => toggleParticipant('task', member.id)} className={`rounded-xl border px-3 py-2 text-sm font-bold ${active ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700'}`}>{member.name}</button>; })}</div></div><div className="md:col-span-2 grid grid-cols-2 gap-2"><button onClick={saveTask} className="btn-gradient justify-center min-h-[44px]"><Save className="w-4 h-4" /> Lagre</button><button onClick={() => setShowTaskForm(false)} className="btn-secondary min-h-[44px] justify-center"><X className="w-4 h-4" /> Avbryt</button></div></div></div>}
