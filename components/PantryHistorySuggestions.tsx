...
import { AlertTriangle, Clock, History, Plus, Sparkles, X, Store } from 'lucide-react';
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                  {suggestions.map(s => {
                    ...
                    const vendors = Object.entries(s.vendorPrices);
                    return (
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1.5 flex flex-wrap gap-1">
                              ...
                              {vendors.slice(0, 3).map(([vendor, info]) => (
                                <span key={vendor} className="inline-flex items-center gap-1 rounded-full bg-slate-100 text-slate-700 px-2 py-0.5 text-[10px] font-bold">
                          ...
                          <button
                            onClick={() => handleAdd(s)}
                            disabled={isAdded}
                            className={`shrink-0 rounded-xl p-2 ${isAdded ? 'bg-emerald-200 text-emerald-800 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-slate-700'}`}
