...
function sourceLabel(event: LiquidityEvent) {
  ...
  if (event.source === 'predicted_from_history') {
    const obs = event.basedOn?.observations ?? 0;
    return obs > 1 ? `Forutsigbar (${obs} obs.)` : 'Forutsigbar (1 obs.)';
  }
...
function confidenceLabel(c: LiquidityEvent['confidence']) {
  return c === 'fixed' ? 'fast' : c === 'predicted' ? 'forutsagt' : 'estimert';
}
