...
export const ResidentsManager: React.FC<Props> = ({ familyMembers, setFamilyMembers, lang }) => {
  ...
  const handleSave = () => {
    ...
    const normalized: FamilyMember = { ...editingMember, salaryDay: cleanSalaryDay(editingMember.salaryDay || 0) };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      {editingMember && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <div className="glass-panel w-full max-w-lg p-10 border-t-4 border-cyan-500 animate-in zoom-in-95 duration-300 relative max-h-[90vh] overflow-y-auto">
      ...
      <div className="flex justify-between items-center mb-10"><div><h3 className="text-sm font-black text-slate-500 uppercase tracking-[0.3em]">Husholdningsregister</h3><p className="text-xs text-slate-400 italic">Oversikt over lønn, ytelser og familiebidrag.</p></div><button onClick={openAdd} className="flex items-center gap-2 px-6 py-3 border border-cyan-500 text-cyan-400 text-[10px] font-black uppercase tracking-widest hover:bg-cyan-500 hover:text-black transition-all shadow-[0_0_15px_rgba(0,243,255,0.2)]"><UserPlus className="w-4 h-4" /> Ny Beboer</button></div>
