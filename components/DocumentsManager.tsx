...
export const DocumentsManager: React.FC<Props> = ({ userId, familyName = 'Familien', familyLocation = '', familyAddress = '' }) => {
  ...
  const loadDocuments = async () => {
    ...
    try {
      ...
      if (resolvedHousehold) {
        ...
        setError('Fant ikke eller klarte ikke å opprette familie/household. Sjekk at Supabase-tabellene households og household_members finnes, og at innlogget bruker har RLS-tilgang.');
      }
    } catch (err: any) {
  ...
  useEffect(() => { loadDocuments(); }, [userId, familyName]);
  ...
  const addDocument = async () => {
    ...
    try {
      ...
      if (!resolvedHousehold) throw new Error('Fant ikke familie/household for dokumentlagring. Gå til Innstillinger → Familie og app, fyll ut familienavn/adresse, og sjekk at Supabase har households-tabeller og RLS-policyer.');
    ...
    } catch (err: any) { setError(err?.message || 'Klarte ikke å lagre dokumentet.'); }
  ...
  const openDocument = async (doc: FamilyDocumentRecord) => {
    ...
    } catch (err: any) { setError(err?.message || 'Klarte ikke å åpne dokumentet.'); }
  ...
  const removeDocument = async (doc: FamilyDocumentRecord) => {
    ...
    try { await deleteFamilyDocument(doc.id, doc.storagePath); }
    catch (err: any) { setDocuments(previous); setError(err?.message || 'Klarte ikke å slette dokumentet.'); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <Card className="p-5"><p className="text-sm text-slate-500">Household</p><p className="mt-1 text-xl font-bold text-slate-900">{familyName}</p><p className="mt-1 text-xs text-slate-500">{familyAddress || familyLocation || 'Adresse/lokasjon ikke satt'}</p></Card>
      ...
      <Card className="p-5"><div className="flex items-start gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-100 text-slate-600"><Upload className="h-5 w-5" /></div><div><h2 className="text-lg font-bold text-slate-900">Sikker filopplasting er aktivert</h2><p className="mt-1 text-sm text-slate-500">Filer lagres i privat Supabase Storage bucket og åpnes via signerte lenker som utløper etter 5 minutter.</p></div></div></Card>
    </div>
  );
};
