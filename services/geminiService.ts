...
const GEMINI_DISABLED_KEY = 'familyhub_gemini_disabled_until';
const GEMINI_DISABLED_REASON_KEY = 'familyhub_gemini_disabled_reason';
...
function resolveAiKey() {
  ...
  return userKey || cleanEnv(viteEnv.VITE_GEMINI_API_KEY) || cleanEnv(viteEnv.VITE_GOOGLE_AI_API_KEY) || cleanEnv(viteEnv.VITE_GOOGLE_GENAI_API_KEY) || cleanEnv(viteEnv.GEMINI_API_KEY) || cleanEnv(processEnv.API_KEY) || cleanEnv(processEnv.GEMINI_API_KEY) || '';
}

function rawError(err: any) {
  return typeof err === 'string' ? err : JSON.stringify(err?.error || err?.message || err || {});
}

function isSuspendedGeminiKey(err: any) {
  const raw = rawError(err).toLowerCase();
  return raw.includes('consumer_suspended') || raw.includes('has been suspended');
}

function disableGemini(reason: string, hours = 24) {
  try {
    localStorage.setItem(GEMINI_DISABLED_KEY, String(Date.now() + hours * 60 * 60 * 1000));
    localStorage.setItem(GEMINI_DISABLED_REASON_KEY, reason);
  } catch {}
}

function geminiDisabledReason() {
  try {
    const until = Number(localStorage.getItem(GEMINI_DISABLED_KEY) || 0);
    if (until && until > Date.now()) return localStorage.getItem(GEMINI_DISABLED_REASON_KEY) || 'Gemini er midlertidig deaktivert.';
    if (until) {
      localStorage.removeItem(GEMINI_DISABLED_KEY);
      localStorage.removeItem(GEMINI_DISABLED_REASON_KEY);
    }
  } catch {}
  return '';
}

async function safeGeminiJson<T>(runner: () => Promise<T>): Promise<T> {
  const disabled = geminiDisabledReason();
  if (disabled) throw new Error(disabled);
  try {
    return await runner();
  } catch (err: any) {
    if (isSuspendedGeminiKey(err)) {
      const message = 'Gemini API-nøkkelen er suspendert i Google-prosjektet. Bytt nøkkel under Innstillinger → AI. Gemini er deaktivert i denne nettleseren i 24 timer.';
      disableGemini(message);
...
export function friendlyAiError(err: any) {
  ...
  if (lower.includes('consumer_suspended') || lower.includes('has been suspended')) return 'Gemini API-nøkkelen er suspendert i Google-prosjektet. Bytt Gemini API-nøkkel under Innstillinger → AI.';
...
const getAi = () => {
  ...
  if (!finalKey || finalKey === 'undefined') console.error("Gemini API_KEY mangler! Legg den inn i Innstillinger.");
  return new GoogleGenAI({ apiKey: finalKey || '' });
};

export const isAiAvailable = () => {
  const finalKey = resolveAiKey();
  return !geminiDisabledReason() && !!finalKey && finalKey !== 'undefined' && finalKey !== '';
};
...
export const getFarmStrategicAdvice = async (ops: FarmOperation[], profile: FarmProfile, tasks: FarmTask[]) => safeGeminiJson(async () => {
  const ai = getAi();
  const response = await ai.models.generateContent({ model: GEMINI_PRO, contents: `Du er en ekspert på landbruksøkonomi og strategisk ledelse for olivenproduksjon i Spania. Analyser Dona Anna-gården basert på følgende data: Profil: ${JSON.stringify(profile)} Operasjoner: ${JSON.stringify(ops)} Fokusområder: lønnsomhet, investeringer, kostnader og markedsføring. Svar på norsk.`, config: { responseMimeType: 'application/json', responseSchema: { type: Type.OBJECT, properties: { strategicSummary: { type: Type.STRING }, profitabilityAnalysis: { type: Type.STRING }, investmentSuggestions: { type: Type.ARRAY, items: { type: Type.STRING } }, costSavingTips: { type: Type.ARRAY, items: { type: Type.STRING } }, marketingIdeas: { type: Type.ARRAY, items: { type: Type.STRING } }, criticalAlerts: { type: Type.ARRAY, items: { type: Type.STRING } }, questionsForUser: { type: Type.ARRAY, items: { type: Type.STRING } } }, required: ['strategicSummary', 'profitabilityAnalysis', 'investmentSuggestions', 'costSavingTips', 'marketingIdeas', 'criticalAlerts', 'questionsForUser'] } } });
  return JSON.parse(response.text || '{}');
});

export const getFinancialStatusInsight = async (stats: any, assets: Asset[]) => safeGeminiJson(async () => {
  const ai = getAi();
  const response = await ai.models.generateContent({ model: GEMINI_FLASH, contents: `Analyser finansene: ${JSON.stringify(stats)}. Svar på norsk, kort og profesjonelt.`, config: { responseMimeType: "application/json", responseSchema: { type: Type.OBJECT, properties: { message: { type: Type.STRING }, sentiment: { type: Type.STRING } }, required: ['message', 'sentiment'] } } });
  return JSON.parse(response.text || '{}');
});

export const getSmartShoppingSuggestions = async (history: string[]) => safeGeminiJson(async () => {
  const ai = getAi();
  const response = await ai.models.generateContent({ model: GEMINI_FLASH, contents: `Forslag basert på: ${history.join(', ')}. Svar på norsk.`, config: { responseMimeType: "application/json", responseSchema: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { name: { type: Type.STRING }, reason: { type: Type.STRING } }, required: ['name', 'reason'] } } } });
  return JSON.parse(response.text || '[]');
});
...
export const generateSmartMenu = async (inv: string[], cra: string) => safeGeminiJson(async () => {
  const ai = getAi();
  const response = await ai.models.generateContent({ model: GEMINI_FLASH, contents: `Lag en ukemeny (7 dager). Lager: ${inv.join(', ')}. Preferanser: ${cra}. Svar på norsk.`, config: { responseMimeType: "application/json", responseSchema: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { day: { type: Type.STRING }, dish: { type: Type.STRING }, reason: { type: Type.STRING }, recipe: { type: Type.OBJECT, properties: { fullIngredients: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { name: { type: Type.STRING }, amount: { type: Type.STRING } } } }, instructions: { type: Type.ARRAY, items: { type: Type.STRING } } } } } } } } });
  return JSON.parse(response.text || '[]');
});
...
export const estimateAssetGrowth = async (type: string, loc: string) => safeGeminiJson(async () => {
  const ai = getAi();
  const response = await ai.models.generateContent({ model: GEMINI_FLASH, contents: `Vekstestimat for ${type} i ${loc}. Svar på norsk.`, config: { responseMimeType: "application/json", responseSchema: { type: Type.OBJECT, properties: { annualGrowthPct: { type: Type.NUMBER }, reasoning: { type: Type.STRING }, historicalContext: { type: Type.STRING } }, required: ['annualGrowthPct', 'reasoning', 'historicalContext'] } } });
  return JSON.parse(response.text || '{}');
});

export const getFarmYieldForecast = async (profile: FarmProfile) => safeGeminiJson(async () => {
  const ai = getAi();
  const response = await ai.models.generateContent({ model: GEMINI_PRO, contents: `Lag en avlingsprognose for 2026 for Dona Anna basert på: ${JSON.stringify(profile)}. Bruk Google Search for Alicante klima-trender.`, config: { tools: [{ googleSearch: {} }], responseMimeType: "application/json", responseSchema: { type: Type.OBJECT, properties: { forecastLiters: { type: Type.NUMBER }, confidenceInterval: { type: Type.STRING }, climaticContext: { type: Type.STRING }, treeHealthAnalysis: { type: Type.STRING }, projectedGrowthPct: { type: Type.NUMBER } }, required: ['forecastLiters', 'confidenceInterval', 'climaticContext', 'treeHealthAnalysis', 'projectedGrowthPct'] } } });
  return JSON.parse(response.text || '{}');
});
...
export const generateZenEcoGuide = async () => safeGeminiJson(async () => {
  const ai = getAi();
  const response = await ai.models.generateContent({ model: GEMINI_PRO, contents: `Du er en ekspert på det spanske boligmarkedet og en topp tekstforfatter for "Zen Eco Homes". Vi selger trygghet, kvalitet og livsstil til nordmenn. Generer 4 deler: titler, struktur, innhold og salgstekst. Svar på norsk.`, config: { thinkingConfig: { thinkingBudget: 4000 } } });
