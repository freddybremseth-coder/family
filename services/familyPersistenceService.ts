...
import { Asset, BankAccount, FamilyMember, Transaction, TransactionType } from '../types';
...
function normalizeTransactionType(value: any): TransactionType {
  const text = String(value || '').trim().toUpperCase();
  if (['INCOME', 'INNTEKT', 'CREDIT', 'PENGER_INN', 'PENGER INN'].includes(text)) return TransactionType.INCOME;
  if (['TRANSFER', 'OVERFØRING', 'OVERFORING'].includes(text)) return TransactionType.TRANSFER;
...
export function mapTransactionRow(row: any): Transaction { return { ...row, amount: numberValue(row.amount), type: normalizeTransactionType(row.type), paymentMethod: row.payment_method || row.paymentMethod || 'Bank', isAccrual: !!(row.is_accrual ?? row.isAccrual), fromAccountId: row.from_account_id || row.fromAccountId || undefined, toAccountId: row.to_account_id || row.toAccountId || undefined, isVerified: !!(row.is_verified ?? row.isVerified), verifiedAt: row.verified_at || row.verifiedAt || undefined, verificationSource: row.verification_source || row.verificationSource || undefined, matchedReceiptId: row.matched_receipt_id || row.matchedReceiptId || undefined, bankStatementRef: row.bank_statement_ref || row.bankStatementRef || undefined } as Transaction; }
...
function fullTransactionRow(userId: string, tx: Transaction) { return { id: cleanId(tx.id, `tx-${Date.now()}-${Math.random().toString(16).slice(2)}`), user_id: userId, date: tx.date || nowIsoDate(), amount: numberValue(tx.amount), currency: tx.currency || 'EUR', description: tx.description || '', category: tx.category || 'Diverse', type: normalizeTransactionType(tx.type), payment_method: safePaymentMethod(tx.paymentMethod), is_accrual: !!tx.isAccrual, from_account_id: tx.fromAccountId || null, to_account_id: tx.toAccountId || null, is_verified: !!tx.isVerified, verified_at: tx.verifiedAt || null, verification_source: tx.verificationSource || null, matched_receipt_id: tx.matchedReceiptId || null, bank_statement_ref: tx.bankStatementRef || null }; }
function snakeLegacyTransactionRow(userId: string, tx: Transaction) { return { id: cleanId(tx.id, `tx-${Date.now()}-${Math.random().toString(16).slice(2)}`), user_id: userId, date: tx.date || nowIsoDate(), amount: numberValue(tx.amount), currency: tx.currency || 'EUR', description: tx.description || '', category: tx.category || 'Diverse', type: normalizeTransactionType(tx.type), payment_method: safePaymentMethod(tx.paymentMethod) }; }
function camelLegacyTransactionRow(userId: string, tx: Transaction) { return { id: cleanId(tx.id, `tx-${Date.now()}-${Math.random().toString(16).slice(2)}`), user_id: userId, date: tx.date || nowIsoDate(), amount: numberValue(tx.amount), currency: tx.currency || 'EUR', description: tx.description || '', category: tx.category || 'Diverse', type: normalizeTransactionType(tx.type), paymentMethod: safePaymentMethod(tx.paymentMethod), isAccrual: !!tx.isAccrual }; }
function minimalTransactionRow(userId: string, tx: Transaction) { return { id: cleanId(tx.id, `tx-${Date.now()}-${Math.random().toString(16).slice(2)}`), user_id: userId, date: tx.date || nowIsoDate(), amount: numberValue(tx.amount), description: tx.description || '', type: normalizeTransactionType(tx.type) }; }
...
export async function loadFamilyPersistentData(userId: string) {
  ...
  const transactions = tx.ok && tx.rows.length > 0 ? tx.rows.map(mapTransactionRow).sort((a, b) => b.date.localeCompare(a.date)) : fallback.transactions.map(mapTransactionRow);
