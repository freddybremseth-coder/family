...
function readBackup<T>(userId: string, name: string): T[] {
  try { const data = JSON.parse(localStorage.getItem(backupKey(userId, name)) || '[]'); return Array.isArray(data) ? data : []; } catch { return []; }
}
function writeBackup(userId: string, name: string, data: any[]) {
  try { if (Array.isArray(data) && data.length > 0) localStorage.setItem(backupKey(userId, name), JSON.stringify(data.slice(0, 5000))); } catch {}
}
...
function isDisabled(key: string) { try { const until = Number(localStorage.getItem(key) || 0); if (until && until > Date.now()) return true; if (until) localStorage.removeItem(key); } catch {} return false; }
function disable(key: string, label: string, error: any) { try { localStorage.setItem(key, String(Date.now() + DISABLE_MS)); } catch {} console.warn(`[familyPersistence] ${label} disabled for 30 minutes after Supabase rejected schema`, error); }
...
async function safeSelect(table: PersistenceTable, userId: string, orderColumn?: string): Promise<SafeSelectResult> {
  ...
  if (result.error) { if (isMissingTable(result.error)) console.warn(`[familyPersistence] ${table} finnes ikke i Supabase. Kjør migration for å aktivere lagring.`, result.error); else console.warn(`[familyPersistence] ${table} load failed`, result.error); return { rows: [], ok: false }; }
  return { rows: result.data || [], ok: true };
}

export async function loadFamilyPersistentData(userId: string) {
  if (!isSupabaseConfigured() || !userId) return { transactions: readBackup<Transaction>(userId, 'transactions'), members: readBackup<FamilyMember>(userId, 'members'), assets: readBackup<Asset>(userId, 'assets'), bankAccounts: readBackup<BankAccount>(userId, 'bank_accounts') };
  ...
  const txBackup = readBackup<Transaction>(userId, 'transactions');
  const memberBackup = readBackup<FamilyMember>(userId, 'members');
  const assetBackup = readBackup<Asset>(userId, 'assets');
  const bankBackup = readBackup<BankAccount>(userId, 'bank_accounts');
  const transactions = tx.ok ? tx.rows.map(mapTransactionRow).sort((a, b) => b.date.localeCompare(a.date)) : txBackup;
  const members = member.ok ? member.rows.map(mapMemberRow) : memberBackup;
  const assets = asset.ok ? asset.rows.map(mapAssetRow) : assetBackup;
  const bankAccounts = bank.ok ? bank.rows.map(mapBankAccountRow) : bankBackup;
  const finalTransactions = transactions.length > 0 ? transactions : txBackup;
  const finalMembers = members.length > 0 ? members : memberBackup;
  const finalAssets = assets.length > 0 ? assets : assetBackup;
  const finalBankAccounts = bankAccounts.length > 0 ? bankAccounts : bankBackup;
  writeBackup(userId, 'transactions', finalTransactions);
  writeBackup(userId, 'members', finalMembers);
  writeBackup(userId, 'assets', finalAssets);
  writeBackup(userId, 'bank_accounts', finalBankAccounts);
  return { transactions: finalTransactions, members: finalMembers, assets: finalAssets, bankAccounts: finalBankAccounts };
}

async function upsertWithFallbacks(table: PersistenceTable, variants: any[]) { let lastError: any = null; for (const rows of variants) { const payload = Array.isArray(rows) ? rows : [rows]; if (payload.length === 0) return null; const { error } = await supabase.from(table).upsert(payload, { onConflict: 'id' }); if (!error) return null; lastError = error; if (isMissingTable(error) || !isSchemaError(error)) break; } return lastError; }
...
export async function deleteTransactionFromSupabase(userId: string, id: string) { if (!userId || !id) return; writeBackup(userId, 'transactions', readBackup<Transaction>(userId, 'transactions').filter((t: any) => t.id !== id)); if (!isSupabaseConfigured()) return; const { error } = await supabase.from('transactions').delete().eq('user_id', userId).eq('id', id); if (error) console.warn('[familyPersistence] transaction delete failed', error); }
export async function syncMembers(userId: string, members: FamilyMember[]) { if (!userId || members.length === 0) return; writeBackup(userId, 'members', members); if (!isSupabaseConfigured() || isDisabled(MEMBER_SYNC_DISABLED_KEY)) return; const error = await upsertWithFallbacks('members', [fullMemberRows(userId, members), snakeLegacyMemberRows(userId, members), camelLegacyMemberRows(userId, members), minimalMemberRows(userId, members)]); if (error) disable(MEMBER_SYNC_DISABLED_KEY, 'member sync', error); }
